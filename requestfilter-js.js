...
class Handlers
{
    ...
    */
    ...
    //新增一项fiddler结果展示 ---  请求耗时项
    public static BindUIColumn("TimeCost/ms", 120)
    function TimeCost(oSession: Session):String{
        
        var sResult = "0";
        var t1_ms = oSession.Timers.ClientBeginResponse.ToUniversalTime().Millisecond;
        var t1_m = oSession.Timers.ClientBeginResponse.ToUniversalTime().Minute;
        var t1_s = oSession.Timers.ClientBeginResponse.ToUniversalTime().Second;
        var t1 = t1_m*60*1000 + t1_s*1000 + t1_ms ;
        var t2_ms = oSession.Timers.ClientDoneRequest.ToUniversalTime().Millisecond;
        var t2_m = oSession.Timers.ClientDoneRequest.ToUniversalTime().Minute;
        var t2_s = oSession.Timers.ClientDoneRequest.ToUniversalTime().Second;
        ...
        //过滤无关请求，只关注特定请求例如url包含“ .lofter.com”  且响应时间大于1000ms的接口 

        if(oSession.fullUrl.Contains("api.lofter.com") && (Number(sResult) > 10 || oSession.responseCode == 500 )){
            
            var oExportOptions = FiddlerObject.createDictionary();
            oExportOptions.Add("ExportToString", "true");
            FiddlerApplication.DoExport("cURL Script", [oSession], oExportOptions, null);
            var curlInfo: String = oExportOptions["OutputAsString"];
            var fsoCurl;
            var fileCurl;
            fsoCurl = new ActiveXObject("Scripting.FileSystemObject");
            //文件保存路径，可自定义
            fileCurl = fsoCurl.OpenTextFile("C:\\Users\\SQ\\Desktop\\代理工具DIY\\res\\curl.txt",8 ,true, true);
            
            var requestInfo:String = "Request url: " + oSession.url + "\n" +
                "Request body: " + oSession.GetRequestBodyAsString() + "\n" + 
                "Response code: " + oSession.responseCode + "\n" +
                "Request timeTake: " + sResult + "\n";
            var clientIP = "clientIP : " + oSession.m_clientIP;
            var clientPort = "clientPort : " + oSession.m_clientPort;
            var m_hostIP = "m_hostIP : " + oSession.m_hostIP;
            var host = "host : " + oSession.hostname;       
            var res = curlInfo + "\n" +requestInfo + "\n" + clientIP + "\n" + clientPort + "\n" + m_hostIP + "\n" + host + "\n";
            fileCurl.writeLine(res); 
            fileCurl.close();                                                    
            
            // 异常接口请求，计划通过接口上报至服务端，存执mongodb,以便后续查询，待后端接口完成后放开
            var method:String = 'POST';
            var url:String = 'http://localhost:8080/qabackend/uploadAbnormalInfo/';
            var protocol = "HTTP/1.1"; 
            var raw:String = '';
            raw += method + " " + url + " " + protocol + "\r\n";
            raw += "Content-Type: application/x-www-form-urlencoded\r\n";
            var body = 'clientIP=' + oSession.m_clientIP +
                '&user=' +"SunQiao"+
                '&hostIP=' + oSession.m_hostIP +
                '&host=' + oSession.hostname +
                '&curl=' + curlInfo +
                '&responseCode=' + oSession.responseCode +
                '&timeTake=' + sResult +
                '&requestUrl=' + oSession.url +
                '&requestHeader='+ oSession.RequestHeaders +
                '&deviceId='+ oSession.RequestHeaders["deviceid"] +
